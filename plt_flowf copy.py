...
class CreSF():
    ...
    def inference_backup(self, curr_left, curr_right, curr_transf_mtx, cre_depth = None, cre_disparity=None, of=None):
    ...
    #Ref: https://github.com/megvii-research/CREStereo/blob/master/test.py
    def inference(self, curr_left, curr_right, curr_transf_mtx, intrinsicss, cre_depth = None, cre_disparity=None, of=None):
        ...
        if len(self.past_rgb_frames) < self.frame_history_length:
            ...
            return sceneflow, vis, flow, vis, depth, vis, flow, vis, flow, vis, sceneflow, vis, sceneflow, vis,
            ...
            (sceneflow, 
                    ...
                    final_sceneflow,
                    final_sceneflow_vis
                    ) = self.infer_sceneflow(depth, flow, self.past_xyz_frames[0], combined_transf_mtx, intrinsicss, self.original_fps)
...
#Ref: https://github.com/megvii-research/CREStereo/blob/master/test.py
    def infer_sceneflow(self, depth, flow, prev_pcd_xyz, combined_transf_mtx, intrinsics, original_fps, device="cuda:0"):
            
            ### decompose the transf matrix (easier to use in warp)
            combined_rotaion_mtx = combined_transf_mtx[:3, :3]
            combined_rotation_vec = R.from_matrix(combined_rotaion_mtx).as_rotvec()
            combined_translation_vec = combined_transf_mtx[:3, 3]
            ...
            index_flow = (grid_mesh - flow).astype(np.int32)
            index_flow[:,:,0] = np.clip(index_flow[:,:,0], 0, intrinsics["width"] - 2.0)
            index_flow[:,:,1] = np.clip(index_flow[:,:,1], 0, intrinsics["height"] - 2.0)
            
            ### compute sceneflow
            curr_xyz = self.compute_xyz_from_depth(depth)
            sceneflow =  curr_xyz - prev_pcd_xyz[index_flow[:,:,1], index_flow[:,:,0]]
            final_sceneflow = sceneflow - (induced_sceneflow)
            ...
            df_mask = np.zeros((intrinsics["height"], intrinsics["width"], 3), dtype=np.uint8)
            df_mask[dynamic_flow_mag > 3.5] = [255, 255, 255]
            # cv2.imshow("df_mask", df_mask)
            ...
            # flow_vis = flow_to_image(flow)
            flow_vis = flow_viz.flow_to_image_clip(flow)
            depth_vis = visualize_depth(torch.from_numpy(depth).to(device), max_depth=100).cpu().numpy()
            sceneflow_vis = tau_to_vis(sceneflow, min_max=2.0)
            induced_sceneflow_vis = tau_to_vis(induced_sceneflow, min_max=2.0)
            final_sceneflow_vis = tau_to_vis(final_sceneflow, min_max=2.0)
            # ic(left.dtype)
            ...
            sceneflow *= original_fps
            induced_sceneflow *= original_fps
            final_sceneflow *= original_fps
    ...
    def compute_induced_flow(self, curr_left, curr_right, curr_transf_mtx, depth = None, cre_disparity=None, of=None):
        ...
        if len(self.past_rgb_frames) < self.frame_history_length:
            ...
            induced_flow, induced_sceneflow = ck.run_generate_induced_flow_img(depth,
                                                                    self.h,
                                                                    self.w,
                                                                    self.fx,
                                                                    self.fy,
    ...
    def compute_dynamic_flow(self, np_flow, depth, rel_rot_vec, rel_transl_vec, intrinsics):
