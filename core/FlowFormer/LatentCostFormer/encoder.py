...
from src.deepLearning.FlowFormer.core.utils.utils import coords_grid, bilinear_sampler, upflow8
...
class CostPerceiverEncoder(nn.Module):
    ...
    def forward(self, cost_volume, data, context=None):
        ...
        if self.cost_scale_aug is not None:
            scale_factor = torch.FloatTensor(B*H1*W1, self.cfg.cost_heads_num, H2, W2).uniform_(self.cost_scale_aug[0], self.cost_scale_aug[1]).to(device="cuda:1")
